...
uint8_t receiveData(char* str)//整体作用：查找到“+IPD,”后的字节数，建立空间存放真正的数据
{
	if(strstr(str,"+IPD,"))//判断字符串中是否存在"+IPD"：存在，继续；不存在，返回。注意：即使+IPD是在str首位，会返回1而不是0
	{
		char* douhao = strchr(str,',');//strchr返回一个char型指针，此指针所指向数据的第一个数据与“，”相同，其后数据与douhao所指向数据一样。
																	 //注意：也就是说，起两个作用：一个是数据分割，另一个是返回的是一个指针（所以是char* douhao）
		char* maohao = strchr(str,':');//同上
		if (douhao && maohao)//douhao与maohao都不为0
		{
			*douhao = 0;//排空maohao指针
			*maohao = 0;//排空douhao指针
			uint8_t len = atoi(douhao+1);//atoi() 函数用来将字符串转换成整数(int)，atoi() 函数会扫描参数 str 字符串，跳过前面的空白字符（例如空格，tab缩进等，
																	//可以通过 isspace() 函数来检测），直到遇上数字或正负符号才开始做转换，而再遇到非数字或字符串结束时('\0')才结束转换，并将结果返回。
																	//注意：函数原型为int atoi (const char * str);  有指针作为函数参数时的地址转换可知，实参为“douhao+1”即可
																	//也就是从“，”后一位开始查找int型数据，放入len中，在这里就是+IPD,后的 传输数据 字节数
			if (g_sim808_receiveString)
			{
				free(g_sim808_receiveString);//先确认g_sim808_receiveString已经释放，防止重复放入，防止所分配空间溢出
			}
			g_sim808_receiveString = malloc(len+1);//为g_sim808_receiveString分配一个len+1长的
			strncpy(g_sim808_receiveString,maohao+1,len);//把g_sim808_receiveString字符串中以maohao+1地址开头的len个数据重新放到把g_sim808_receiveString字符串中
...
uint8_t smart(int8_t type)
{
	if (type == 0)
	{
		setCommandWait(10,"CWSTARTSMART",NULL);
	}
	else
	{
		char buffer[4];
		sprintf(buffer,"%d",type);
		setCommandWait(10,"CWSTARTSMART",buffer,NULL);
	}
	if (waitUntilOk(5000))
	{
		return 1;
...
//	char* status = getCommand(1000,"CIPSTATUS");
	if (!g_sim808_smartStarted)
	{
		...
		else if (status == 3)
		{
			...
			setCommandWait(10,"CIPSENDEX",buffer,NULL);
			if(waitUntilOk(500))
			{
				printf(str);
				waitUntilOk(5000);
...
//			char buffer[0x10];
//			sprintf(buffer,"%d",strlen(str));
//			setCommandWait(10,"CIPSEND",buffer,NULL);
			setCommandWait(10,"CIPSEND",NULL);
			hw_delay(100);
...
uint8_t sim808_disconncet(uint8_t force)
{
	if (!g_sim808_smartStarted)
	{
		...
		if(force || status == 3)
		{
			setCommandWait(10,"CIPCLOSE",NULL);
			if(waitUntilOk(500))
			{
				return 1;
			}
...
//	char* ssid = "\"AIRJ_NB\"";
//	setCommandWait(20000,"CWJAP_DEF",ssid,"\"airj2123\"",NULL); //default ssid.
//	setCommandWait(10,"CIUPDATE",NULL);
//	if(waitUntilStrcmp(5000,"+CIPUPDATE:4"))
//	{
//		waitUntilStrstr(60000,"jump to run user");
//	}
	
//	setCommandWait(10,"CWSAP","\"AIRJESPAIRJ\"","\"airj2123\"","11","3",NULL);
}
...
//		setCommandWait(1,"CGNSPWR","1",NULL);
		setCommandWait(10,"CIPHEAD","1",NULL); //should befor sapbr,or will not exe sometime
		setCommandWait(10,"SAPBR","3","1","\"Contype\"","\"GPRS\"",NULL);
		setCommandWait(10,"SAPBR","3","1","\"APN\"","\"CMNET\"",NULL);
		setCommandWait(10,"SAPBR","1","1",NULL);
		if (waitUntilOk(5000))
		{
		...
		{
			setCommandWait(10,"SAPBR","0","1",NULL);
...
//	setCommandWait(1,"GSN",NULL); //AT+GSN
	...
	while(cnt)
	{
		...
		if(IMEIstr)
		{
			if((char)(*IMEIstr)>=48 && (char)(*IMEIstr)<=57)  //first character is between '0' and '9' .
			{
...
//				decrypt[20] = '\0';
//				sprintf(g_sim808_Info.IMEI_number,"M2-");  //2015-12-7
